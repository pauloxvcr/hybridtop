...
#-------------compilados para bidimensionais----------------------------
@njit(nogil = True)
def LocalKbi(eNode,Node, W,SdNdxi, Nu0,Ec,Et, sigmap, theta):
    ...
    for q in range(0, len(W)):
        ...
        DB = DB + temp  # pode alterar para ser mais rapido
    return Ke, DB
...
def calctheta(sigma):
    ...
    return sigmap, theta
# ---------------- compilados para barra ----------------
@njit(nogil = True)
def calcdeformation(T,l,Ue):
    Ul = T @ Ue
    e = (Ul[1]-Ul[0])/l
    if e[0] <=0: # precisou modificar pq o numba buga com array mesmo de 1 elemento
        BarType = 0
    else:
        BarType = 1
    return BarType
